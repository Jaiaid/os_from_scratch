...
void _MEM_init_kernel(pagedir_entry *dir)
{
	...
	table[7].content=0x00007001;
    	table[8].content=0x00008001;
    	table[9].content=0x00009001;
	for(virtual_page_no=0xa,ptr=0xa000;virtual_page_no<20;virtual_page_no++,ptr+=0x1000)
	{
	    table[virtual_page_no].content=(ptr|0x00000003);
	}
	table[0xb8].content=(0xb8003);
	table[0xb9].content=(0xb9003);
    table[0x7f].content=(0x7f003);
...
{
	dir[entryno].addrs = addrs|0x00000003;
}
	...
	for(l=0;l<MEMPAGE/4;l++)
	{
		for(l1=0;l1<32;l1++)
		{
			if((memmap[l]&(1<<l1))==0){
				...
				return ((l<<5)+l1)<<12; //return base address of allocated page;
			}
		}
	}
